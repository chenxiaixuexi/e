...
$db = mysqli_connect("localhost","root","","e-store");
...
function add_Cart(){
	...
	if(isset($_GET['add_cart'])){
		...
		$check_product = "select * from cart where ip='$ip_add' AND pro_id='$p_id'";
		...
		if (mysqli_fetch_array($run_check) > 0) {
			...
			$query = "insert into cart(pro_id,ip,qty,type)values('$p_id','$ip_add','$product_qty','$product_type')";
...
function getPro(){
	...
	$get_products="select * from product order by 1 DESC LIMIT 0,2";
...
function getpcatpro(){
	...
	if(isset($_GET['p_cat'])){
		$p_cat_id = $_GET['p_cat'];
        $int=intval($p_cat_id);
		$get_p_cat = "select * from product_category where p_cat_id='$int'";
		$run_p_cat = mysqli_query($db,$get_p_cat);
		$row_p_cat = mysqli_fetch_array($run_p_cat);
		$p_cat_title = $row_p_cat['p_cat_title'];
		$p_cat_desc = $row_p_cat['p_cat_desc'];

		$get_products = "select * from product where p_category_id='$p_cat_id'";
...
function getcatpro(){
	...
	if(isset($_GET['cat'])){
		...
		$get_products = "select * from product where category_id='$cat_id'";
...
function items(){
	...
	$ip_add = getRealIpUser();
	$get_items = "select * from cart where ip='$ip_add'";
...
function total_price(){
	...
	$ip_add = getRealIpUser();
	$total = 0;
	$select_cart = "select * from cart where ip='$ip_add'";
